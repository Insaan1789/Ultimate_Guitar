...
// const stringBtns = document.querySelectorAll('.string-btn'); // Moved to dynamic selector
...
if (savedString) {
    if (savedString === 'AUTO') {
        ...
        // Must delay this until strings are rendered?
        // Initial defaults are rendered.
        // If saved string is not in default E2..E4 list, we might have issue if custom was loaded.
        // We'll trust logic flow.
        const btns = document.querySelectorAll('.string-btn');
        const btn = Array.from(btns).find(b => b.dataset.note === savedString);
...
function selectString(btn) {
    ...
    statusMsg.textContent = `Target: ${currentString} (${guitarStrings[currentString].freq.toFixed(1)} Hz)`;
...
// --- Reference Tones ---
// Initial bind (refresh later in renderStringButtons)
        ...
        customTunings.forEach(t => {
            ...
            mainClickDiv.onclick = (e) => {
                ...
                if (t.headstock) {
                    ...
                    // Re-render strings to match custom layout preference
...
headstockBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (!isChromaticMode) {
            ...
            else if (activeInstrument === 'custom') {
                // For custom, we probably want to render the current custom ones
                const t = customTunings.find(c => c.name === activeTuningName);
                if (t) renderStringButtons(t.notes);
            }
        }
    });
});
